...
class DQN():
  ...
  def build_net(self, path, add=0):
    ...
    if add:
      ...
      W1 = np.append(W1_old,W1_add_zero,axis=0).astype(np.float32)
      b1 = b1_old.astype(np.float32)
      W2 = np.append(W2_old,W2_add_zero,axis=1).astype(np.float32)
      b2 = np.append(b2_old,b2_add_zero,axis=0).astype(np.float32)
      ...
      Q_action = tf.reduce_sum(tf.multiply(self.Q_value,self.action_input),reduction_indices = 1)
      self.cost = tf.reduce_mean(tf.square(self.y_input - Q_action))
      self.optimizer = tf.compat.v1.train.AdamOptimizer(0.0001).minimize(self.cost,var_list=[W1,b1,W2,b2].tolist())
