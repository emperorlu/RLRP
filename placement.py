...
#     agent.build_net(Ipath)
    for episode in range(1):
        ...
        while i < num:
            ...
            if (stk <= 1.5 and back == 0):
                i += 1
                # agent.epsilon = 1
            elif (stk <= 1.5 and agent.epsilon <= 0): 
                i += 1
                back = 0
                # agent.epsilon = 1
            else: 
...
def DQNTestSigle():
    env = park.make('replica_placement')
    agent = DQN(env,e=0,model=0)
    path = "./dqn_model_1024/{}.ckpt".format(config.num_servers)
    agent.build_net(path)
    
    Rnum = config.num_rep
    final_map = []
    for episode in range(1): #TEST
        state = env.reset()
        done = False
        print("weight: ",env.weight)
        t0 = time.time()
        ...
        me = np.mean(fstate)
        print("total episode:",i,"; cost time: ", t1-t0)
        print("\nstd:",np.std(state), "; over: ", (max(fstate)-me)/me, "; max: ", max(fstate), "; mean: ", me)
        print("episode:",episode, " epsilon:", agent.epsilon,"\nstate: ", state, "\nservers:", fstate)
    ...
    print("\nservers:", hstate)
    me = np.mean(hstate)
    print("\nstd:",np.std(hstate), "; over: ", (max(hstate)-me)/me, "; max: ", max(hstate), "; mean: ", me)
    pg_num = 3333
    t2 = time.time()
